...
def build_items(data, thumbs_map, genre_map, epg_window, fav_ids=None):
    log(f"[BUILD ITEMS] Received {len(data.get('categories', [{}])[0].get('channels', []))} channels for building items")
    log(f"[BUILD ITEMS] Received {fav_ids} channels for building items")
    log(f"[BUILD ITEMS] Received {type(fav_ids)} channels for building items")
    ...
    log(f"[BUILD ITEMS] genre_filter {genre_filter}")
