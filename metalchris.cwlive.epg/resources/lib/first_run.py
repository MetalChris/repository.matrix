...
log(f"[FIRST RUN] User-Agent: {(ua)}", xbmc.LOGDEBUG)
...
def run_first_run():
	...
	if os.path.exists(EPG_JSON):
		log(f"[FIRST RUN] Cached epg.json exists", xbmc.LOGDEBUG)
		return
	"""
	...
	try:
		...
		xbmcgui.Dialog().notification("CW Live EPG", "First run setup complete", xbmcgui.NOTIFICATION_INFO, 3000, sound=False)
