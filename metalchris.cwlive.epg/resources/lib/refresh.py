...
def refresh_epg_list(epg_window):
    try:
        ...
        if heading_ctrl:
            view_state = epg_window.getProperty("VIEW_STATE")
            if view_state == "favorites":
                fav_filter = epg_window.getProperty("FAVORITES_FILTER")
                fav_ids = [s.strip() for s in fav_filter.split(",") if s.strip()] if fav_filter else []
                heading_ctrl.setLabel(f"CW Live - Favorites ({len(fav_ids)} Channels)")
            else:
                # existing logic for genre or full list
                genre_filter = epg_window.getProperty("FILTER_GENRE")
                if genre_filter:
                    heading_ctrl.setLabel(f"CW Live EPG - {genre_filter.capitalize()} ({len(epg_window.listitems)} Channels)")
                else:
