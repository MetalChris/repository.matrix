...
def handle_context_menu(epg_window, listitem):
	...
	try:
		...
		if sel == "Show Program Info":
			...
			times = str(listitem.getProperty("now_start") + ' - ' + str(listitem.getProperty("now_end"))) or "No description available."
			...
			times2 = str(listitem.getProperty("next_start") + ' - ' + str(listitem.getProperty("next_end"))) or "No description available."
			...
			slug = listitem.getProperty("channel_slug")
			chan_id = listitem.getProperty("channel_id")
			chan_name = listitem.getProperty("channel")
			logo = listitem.getArt("icon")  # optional
			url = listitem.getProperty("url")  # optional
			if add_favorite(slug, chan_id, chan_name, logo, url):
			...
			epg_window.setProperty("EPG_TITLE", "LG Channels EPG")
			...
			dlg = xbmcgui.Dialog()
			confirm = dlg.yesno("LG Channels EPG", "Are you sure you want to delete all favorites?")
			if not confirm:
			...
			try:
				...
				genres = sorted(list(genre_map.keys()))
				...
				if selected_genre == "Show All Channels":
					...
					channels_for_genre = genre_map.get(selected_genre, [])
