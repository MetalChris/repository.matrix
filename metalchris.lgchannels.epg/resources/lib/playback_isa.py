...
def play_episode_isa(title, url, image, epg_window=None):
	try:
		...
		if not url:
			xbmcgui.Dialog().notification("LG Channels EPG", "No stream URL", xbmcgui.NOTIFICATION_ERROR, 3000, sound=False)
			return
		...
		if title:
			...
			log(f"[PLAYBACK_ISA] Window Title: {title}", xbmc.LOGINFO)
