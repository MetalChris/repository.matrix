...
class PlayerMonitor(xbmc.Player):
    ...
    def __init__(self, epg_window):
        ...
        log("[LocalNow EPG][MONITOR] PlayerMonitor initialized and listening for playback events", xbmc.LOGINFO)

    def onPlayBackStopped(self):
        log("[LocalNow EPG] [MONITOR] Playback stopped — restoring EPG title and focus", xbmc.LOGINFO)
        self.restore_epg_state()

    def onPlayBackEnded(self):
        log("[LocalNow EPG] [MONITOR] Playback ended — restoring EPG title and focus", xbmc.LOGINFO)
        self.restore_epg_state()
    ...
    def restore_epg_state(self):
        ...
        if not self.epg_window:
            log("[LocalNow EPG] [MONITOR] No EPG window reference — cannot restore state", xbmc.LOGERROR)
            return
        ...
        try:
            ...
            if not restored:
                log("[LocalNow EPG] [MONITOR] Could not restore focus — no matching slug or valid index", xbmc.LOGWARNING)

            # Final verification log
            heading_label = heading_ctrl.getLabel() if heading_ctrl else "N/A"
            log(f"[LocalNow EPG] [MONITOR] Verified/reapplied EPG title: {heading_label}", xbmc.LOGINFO)

        except Exception as e:
            log(f"[LocalNow EPG] [MONITOR] Error restoring EPG state: {e}", xbmc.LOGERROR)
        ...
        try:
            for i in range(ctrl.size()):
                ...
                if item and item.getProperty("channel_id") == slug:
                    ctrl.selectItem(i)
                    log(f"[LocalNow EPG] [MONITOR] Focus restored to slug '{slug}' at index {i}", xbmc.LOGINFO)
                    return True
        except Exception as e:
            log(f"[LocalNow EPG] [MONITOR] _restore_focus_by_slug failed: {e}", xbmc.LOGERROR)
        return False
