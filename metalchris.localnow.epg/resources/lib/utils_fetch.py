...
ADDON = Addon("metalchris.localnow.epg")
FEED_URL = "https://localnow.com/_next/data/ROBNqSuEs-w826kXGvjsa/channels.json"
baseUrl = 'https://localnow.com/'
LN_URL = get_ln(baseUrl)
log(f"[UTILS FETCH] LN_URL: {LN_URL}", xbmc.LOGINFO)
...
def build_genre_map(data):
	...
	try:
		...
		for channel in channels:
			genre = channel.get("genres", "") if isinstance(channel, dict) else ""
			channel_number = channel.get("channel_number", "") if isinstance(channel, dict) else ""
			try:
				genre_map[int(channel_number)] = genre
			except Exception:
				continue
...
def fetch_channel_descriptions(data):
	...
	try:
		...
		log(f"[UTILS FETCH] Built desc_map with {len(desc_map)} entries", xbmc.LOGINFO)
	except Exception as e:
...
def get_channel_thumbs(data):
    """Download channel channel logos only once."""
    try:
        channels = data.get("channels", [])
        if not channels:
            xbmc.log("[UTILS_FETCH] No channels found in feed", xbmc.LOGWARNING)
            return

        # Build a quick list of missing thumbnails
        missing = []
        for channel in channels:
            if not isinstance(channel, dict):
                continue

            channel_number = str(channel.get("channel_number", "")).strip()
            img_url = channel.get("wallpaper", "").strip()

            if not channel_number or not img_url:
                continue

            local_file = os.path.join(THUMBS_DIR, f"{channel_number}.png")
            if not xbmcvfs.exists(local_file):
                missing.append((channel_number, img_url))

        if not missing:
            xbmc.log("[UTILS_FETCH] All logos already cached — skipping download", xbmc.LOGINFO)
            return
        ...
        for i, (channel_number, img_url) in enumerate(missing, 1):
            if dlg.iscanceled():
                break
            ...
            try:
                ...
                with xbmcvfs.File(local_file, "wb") as out:
                    out.write(data_bytes)
            except Exception as e:
                xbmc.log(f"[UTILS_FETCH] Failed to download {img_url}: {e}", xbmc.LOGWARNING)

            # Update every 10 or at the end
        ...
        xbmc.log(f"[UTILS_FETCH] Cached {total} new channel channel logos", xbmc.LOGINFO)

    except Exception as e:
        xbmc.log(f"[UTILS_FETCH] Error building channel logos cache: {e}", xbmc.LOGERROR)
...
def _get_feed():
	...
	try:
		response = s.get(LN_URL, headers = {'User-Agent': ua})
