...
def build_favorites(data, thumbs_map, desc_map, category_map, epg_window, fav_ids=None):
	"""
	...
	for count, item in enumerate(data['channels']):
		...
		#if idx % 2 == 0:
			#li.setProperty("rowbg", "special://home/addons/metalchris.tcltv.epg/resources/media/row_light.png")
			#parity = "EVEN"
		#else:
			#li.setProperty("rowbg", "special://home/addons/metalchris.tcltv.epg/resources/media/row_dark.png")
			#parity = "ODD"
		#li.setProperty("row_parity", parity)
		#log(f"[BUILD FAVORITES] Adding item: {li.getProperty('channel')}", xbmc.LOGINFO)
		items.append(li)
	...
	log(f"[BUILD FAVORITES] Window title set to: {title}", xbmc.LOGINFO)
