...
class PlayerMonitor(xbmc.Player):
	...
	def __init__(self, epg_window):
		...
		log("[TCLTV+ EPG] [MONITOR] PlayerMonitor initialized and listening for playback events", xbmc.LOGINFO)

	def onPlayBackStopped(self):
		log("[TCLTV+ EPG] [MONITOR] Playback stopped — restoring EPG title and focus", xbmc.LOGINFO)
		self.restore_epg_state()

	def onPlayBackEnded(self):
		log("[TCLTV+ EPG] [MONITOR] Playback ended — restoring EPG title and focus", xbmc.LOGINFO)
		self.restore_epg_state()
	...
	def restore_epg_state(self):
		...
		if not self.epg_window:
			log("[TCLTV+ EPG] [MONITOR] No EPG window reference — cannot restore state", xbmc.LOGERROR)
			return
		...
		try:
			...
			if heading_ctrl:
				...
				log(f"[TCLTV+ EPG] [MONITOR] Restored EPG title: {prop_title}", xbmc.LOGDEBUG)
			else:
				log("[TCLTV+ EPG] [MONITOR] Heading control not found — could not restore title", xbmc.LOGWARNING)
				...
				if last_index is not None:
					try:
						...
						if 0 <= index < ctrl.size():
							...
							log(f"[TCLTV+ EPG] [MONITOR] Focus restored to index {index} (fallback)", xbmc.LOGDEBUG)
							restored = True
					except Exception as e:
						log(f"[TCLTV+ EPG] [MONITOR] Failed to restore focus by index: {e}", xbmc.LOGERROR)

			if not restored:
				log("[TCLTV+ EPG] [MONITOR] Could not restore focus — no matching slug or valid index", xbmc.LOGWARNING)

			# Final verification log
			heading_label = heading_ctrl.getLabel() if heading_ctrl else "N/A"
			log(f"[TCLTV+ EPG] [MONITOR] Verified/reapplied EPG title: {heading_label}", xbmc.LOGDEBUG)

		except Exception as e:
			log(f"[TCLTV+ EPG] [MONITOR] Error restoring EPG state: {e}", xbmc.LOGERROR)
	...
	def _restore_focus_by_slug(self, ctrl, slug):
		...
		"""
		log(f"[SLUG DEBUG] Looking for slug: {slug}", xbmc.LOGWARNING)
		...
		try:
			for i in range(ctrl.size()):
				...
				if item and item.getProperty("channel_slug") == slug:
					ctrl.selectItem(i)
					log(f"[TCLTV+ EPG] [MONITOR] Focus restored to slug '{slug}' at index {i}", xbmc.LOGDEBUG)
					return True
		
				
		except Exception as e:
			log(f"[TCLTV+ EPG] [MONITOR] _restore_focus_by_slug failed: {e}", xbmc.LOGERROR)
		return False
