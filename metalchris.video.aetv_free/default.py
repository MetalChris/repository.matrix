...
SHOWS_URL = "https://play.aetv.com/shows"

#xbmc.log("AETV ADDON STARTED", xbmc.LOGERROR)
	...
	try:
		...
		for node in apollo.values():
			if isinstance(node, dict) and node.get("__typename") == "Series":
				log(
					f"AETV SERIES CANDIDATE: id={node.get('id')} canonical={node.get('canonical')} unlocked={len(node.get('unlockedEpisodes', [])) if node.get('unlockedEpisodes') else 0}",
					xbmc.LOGDEBUG
				)
		...
		for key, item in apollo.items():
			...
			#xbmc.log(f"AETV URL: {url}", xbmc.LOGERROR)
...
def list_episodes(slug):
	...
	try:
		url = f"https://play.aetv.com/shows/{slug}"
...
def get_stream_from_dai(content_source_id, video_id):
	...
	log(f"DAI RESPONSE: HTTP {r.status_code} {r.reason}", xbmc.LOGINFO)
	...
	log(f"DAI STREAM URL: {stream_url}", xbmc.LOGINFO)
...
def play_episode(video_id, canonical):
	...
	try:
		episode_url = f"https://play.aetv.com{canonical}"
