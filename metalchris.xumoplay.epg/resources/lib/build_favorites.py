...
def build_favorites(data, thumbs_map, desc_map, genre_map, epg_window, fav_ids=None):
	...
	for count, item in enumerate(data['channels']):
		...
		slug = item['slug']
		url ='https://data-store-trans-cdn.api.cms.amdvids.com/video/play/' + str(videoId) + '/1920/1080?page_url=https%253A%252F%252Fxumoplay.com%252Fchannels%252F' + str(slug) + '&device_devicetype=desktop_web&app_version=0.0.0'
		streamUrl = 'plugin://plugin.video.xumoplay?mode=6&url=' + urllib.parse.quote_plus(url) + '&name=' + urllib.parse.quote_plus(title)
