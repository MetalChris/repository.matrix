...
log(f"[GET_ITEMS] User-Agent: {(ua)}", xbmc.LOGINFO)
...
selfAddon = xbmcaddon.Addon(id='metalchris.xumoplay.epg')
translation = selfAddon.getLocalizedString
addon = xbmcaddon.Addon()
addonname = addon.getAddonInfo('name')
settings = xbmcaddon.Addon(id="metalchris.xumoplay.epg")
local_string = xbmcaddon.Addon(id='metalchris.xumoplay.epg').getLocalizedString
defaultimage = 'special://home/addons/metalchris.xumoplay.epg/resources/media/icon.png'
...
def fetch_all_programs(data):
    ...
    try:
        ...
        log(f"[GET_ITEMS] Saved {len(map_all_programs)} channels", xbmc.LOGINFO)
    except Exception as e:
...
def local_now(title, stream, image, captions, epg_window):
	...
	if 'hasEmbeddedCaptions' in data['providers'][0]['sources'][0]:
		log(('***** CAPTIONS *****'),level=xbmc.LOGDEBUG)
	...
	log('NOT STREAM: ' + str(url),level=xbmc.LOGDEBUG)
