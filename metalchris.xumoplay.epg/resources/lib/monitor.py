...
class PlayerMonitor(xbmc.Player):
    ...
    def restore_epg_state(self):
        ...
        if not self.epg_window:
            log("[XumoPlay EPG] [MONITOR] No EPG window reference — cannot restore state", xbmc.LOGERROR)
            return
        ...
        try:
            ...
            if heading_ctrl:
                ...
                log(f"[XumoPlay EPG] [MONITOR] Restored EPG title: {prop_title}", xbmc.LOGINFO)
            else:
                log("[XumoPlay EPG] [MONITOR] Heading control not found — could not restore title", xbmc.LOGWARNING)
                ...
                if last_index is not None:
                    try:
                        ...
                        if 0 <= index < ctrl.size():
                            ...
                            log(f"[XumoPlay EPG] [MONITOR] Focus restored to index {index} (fallback)", xbmc.LOGINFO)
                            restored = True
                    except Exception as e:
                        log(f"[XumoPlay EPG] [MONITOR] Failed to restore focus by index: {e}", xbmc.LOGERROR)

            if not restored:
                log("[XumoPlay EPG] [MONITOR] Could not restore focus — no matching slug or valid index", xbmc.LOGWARNING)

            # Final verification log
            heading_label = heading_ctrl.getLabel() if heading_ctrl else "N/A"
            log(f"[XumoPlay EPG] [MONITOR] Verified/reapplied EPG title: {heading_label}", xbmc.LOGINFO)

        except Exception as e:
            log(f"[XumoPlay EPG] [MONITOR] Error restoring EPG state: {e}", xbmc.LOGERROR)
        ...
        try:
            for i in range(ctrl.size()):
                ...
                if item and item.getProperty("SLUG") == slug:
                    ctrl.selectItem(i)
                    log(f"[XumoPlay EPG] [MONITOR] Focus restored to slug '{slug}' at index {i}", xbmc.LOGINFO)
                    return True
        except Exception as e:
            log(f"[XumoPlay EPG] [MONITOR] _restore_focus_by_slug failed: {e}", xbmc.LOGERROR)
        return False
