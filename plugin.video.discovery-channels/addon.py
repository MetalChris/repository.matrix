...
import urllib.request, urllib.parse, urllib.error, urllib.request, urllib.error, urllib.parse, xbmcplugin, xbmcaddon, xbmcgui, platform, re, sys, os
import requests
import json
import mechanize
...
defaultimage = 'special://home/addons/plugin.video.discovery-channels/resources/media/icon.png'
defaulticon = 'special://home/addons/plugin.video.discovery-channels/resources/media/icon.png'
defaultfanart = 'special://home/addons/plugin.video.discovery-channels/resources/media/fanart.jpg'
...
def channels():
	...
	if idsc!='false':
		image = artbase + 'investigationdiscovery.jpg'
		addDir2('Investigation Discovery', 'https://www.investigationdiscovery.com', 25, image)
...
def dsc_menu_new(url):
	...
	dscjson = re.compile('type="application/json">(.+?)</script>').findall(page.decode('utf-8'))[0]
...
def diy_menu_new(url):
	...
	xbmc.log('DSCJSON: ' + str(len(dscjson)),level=log_level)
	for contentBlock in range(total):
		if 'Stream for Free' in (data['layout']['contentBlocks'][i]['label']['line1']) or 'Free Episodes' in (data['layout']['contentBlocks'][i]['label']['line1']) or 'Featured Episodes' in (data['layout']['contentBlocks'][i]['label']['line1']) or 'Unlocked Episodes' in (data['layout']['contentBlocks'][i]['label']['line1']):
			xbmc.log('UNLOCKED: ' + str(i),level=log_level)
			value = i
		else:
			i=i+1
		#xbmc.log('LABEL: ' + str(data['layout']['contentBlocks'][i]['label']['line1']),level=log_level)
...
def add_directory3(name,url,mode,fanart,thumbnail,site,plot):
	u=sys.argv[0]+"?url="+urllib.parse.quote_plus(url)+"&mode="+str(mode)+"&name="+urllib.parse.quote_plus(name)+"&site="+urllib.parse.quote_plus(site)
	ok=True
	liz=xbmcgui.ListItem(name)
	liz.setInfo(type="Video", infoLabels={"Title": name, "plot": plot})
	if not fanart:
...
try:
	...
	pass
try:
	site = urllib.parse.unquote_plus(params["site"])
except:
	pass
