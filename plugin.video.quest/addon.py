...
baseurl = 'https://watch.questnetworktv.com/'
...
jsonurl = 'https://prod-api-cached-2.viewlift.com/content/pages?path=%2F&site=quest&includeContent=true'

prefix = 'https://prod-api-cached-2.viewlift.com/content/pages?path=%2Fseries%2F'
suffix = '&site=quest&includeContent=true&moduleOffset=0&moduleLimit=4&languageCode=default&countryCode=US'

defaultimage = 'special://home/addons/plugin.video.quest/resources/media/icon.png'
defaultfanart = 'special://home/addons/plugin.video.quest/resources/media/fanart.jpg'
defaulticon = 'special://home/addons/plugin.video.quest/resources/media/icon.png'
addon_handle = int(sys.argv[1])

headers = {'Host': 'prod-api.viewlift.com', 'User-Agent': ua, 'Referer': 'https://watch.questtv.com/', 'x-api-key': 'PBSooUe91s7RNRKnXTmQG7z3gwD2aDTA6TlJp6ef'}
...
def all_shows():
	...
	for module in range(total):
		if 'contentType' in (data['modules'][i]):
			...
			if 'Series' in contentType:
				if data['modules'][i]['title'] == 'Popular Series':
					xbmc.log('Popular Series',level=log_level)
	...
	for show in range(shows):
		...
		permalink = 'https://watch.questnetworktv.com' + data['modules'][m]['contentData'][i]['gist']['permalink']
...
def single(data):
	...
	for episode in range(total):
		...
		#url = 'plugin://plugin.video.quest?mode=99&url=' + urllib.parse.quote_plus(str(m3u8))
...
def get_stream(name,url):
	r = requests.get('https://prod-api.viewlift.com/identity/anonymous-token?site=quest',headers=headers)
