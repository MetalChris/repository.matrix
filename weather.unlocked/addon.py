...
settings = xbmcaddon.Addon(id="weather.unlocked")
...
LOC_URL          = 'https://nominatim.openstreetmap.org/search?addressdetails=1&q=%s&format=jsonv2'
...
def location(loc):
	...
	for i in range(total):
		listitem   = data[i]['display_name']
		location = listitem
		xbmc.log('LOCATION: ' + str(location),level=log_level)
		lat = data[i]['lat']
		xbmc.log('LAT: ' + str(lat),level=log_level)
		lng = data[i]['lon']
		xbmc.log('LON: ' + str(lng),level=log_level)
